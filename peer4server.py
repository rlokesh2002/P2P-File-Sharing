...
def create_socket():
    try:
        ...
        host = ""
        server_port = 4080 #peer1 server port numbers
    ...
    while True:
        ...
        if re.findall('^file_inquiry', client_response):
            ...
            if len(file_path) > 0:
               ...
               #num_chunks = math.ceil(len(resp)/(1.0*L))

               #s.sendto(str.encode("Chunk_Size%d" % num_chunks),client_address)  # Send reponse to client about the number of chunks



               str_chunk = resp[:min(len(resp), L)]
               resp = resp[min(L, len(resp)):]

               while len(resp)>0:
                    print("Flag: 0")
                    s.sendto(str.encode("0%s" % str_chunk), client_address)  # Send reponse to client. 0 prefix says that there is still string left
                    str_chunk = resp[:min(len(resp), L)]
                    resp = resp[min(L, len(resp)):]

               print("Flag: 1")
               s.sendto(str.encode("1%s" % str_chunk), client_address)  # Send reponse to client. 1 prefix says that this is the last string left
